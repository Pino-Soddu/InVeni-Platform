...
namespace Inveni.App.Modelli
{
    public class Gioco
    {
        ...
        public ImageSource? FotoSemplice
        {
            get
            {
                // Controllo base: se non c'è nome file, restituisci null
                if (string.IsNullOrEmpty(photo1))
                {
                    return null;
                }

                // MAUI cerca automaticamente in Resources/Images/
                // Il percorso corretto è "Images/nomefile.ext"
                return ImageSource.FromFile($"Images/{photo1}");
            }
        }
